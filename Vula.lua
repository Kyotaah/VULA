...
function Vula:CreateWindow(opts)
    ...
    local function open()
        ...
        shadow.Visible=true
        arrowLbl.Text="‹"
        tw(Main,{Size=OPEN_SIZE,Position=OPEN_POS},.3,Enum.EasingStyle.Back)
        task.delay(.32,function() Deb=false end)
    end
    local function close(silent)
        if Deb then return end; Deb=true; Hidden=true
        arrowLbl.Text="›"
        tw(Main,{Size=UDim2.new(0,WW,0,WH*.88),Position=UDim2.new(.5,3,.5,-38+10)},.18,Enum.EasingStyle.Quint,Enum.EasingDirection.In)
        task.delay(.2,function()
            Main.Visible=false; shadow.Visible=false
            Main.Size=OPEN_SIZE; Main.Position=OPEN_POS
            Deb=false
        end)
        if not silent then Vula:Notify({Title="Hidden",Content="Side tab or RightShift to reopen.",Duration=3,Type="info"}) end
    end
    ...
    -- TextButton hitbox over SideTab — reliable click detection on all platforms
