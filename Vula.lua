...
local function ni(cls, par, props)
    ...
    if props then
        ...
        -- Tag theme-color properties with their key for SetTheme remapping
        if props._bg  then i:SetAttribute("_thBg",  props._bg)  end
        if props._tc  then i:SetAttribute("_thTc",  props._tc)  end
        if props._sc  then i:SetAttribute("_thSc",  props._sc)  end  -- stroke color
        if props._ic  then i:SetAttribute("_thIc",  props._ic)  end  -- image color
    end
    if par then i.Parent = par end
    return i
end
-- Shorthand: tag BackgroundColor3 with a theme key
local function niB(cls, par, props, bgKey, tcKey)
    local i = ni(cls, par, props)
    if bgKey then i:SetAttribute("_thBg", bgKey) end
    if tcKey then i:SetAttribute("_thTc", tcKey) end
    return i
end
...
Vula.Theme = {
    ...
    -- Paper / newspaper
    Paper = {
        Bg="f8f4ec", Top="ede8dc", Side="ece8dc",
        Card="fffdf7", CardH="f5f0e4", Stroke="d8d0bc",
        Text="28220c", Dim="907860", SecLbl="705030",
        Acc="805828", AccD="503810",
        TOn="805828", TOff="d8cdb8", Knob="fffdf7",
        TBOn="805828", TBOff="f8f4ec", TxtOn="fffdf7", TxtOff="a89070",
        Div="d8d0bc", InBg="ede8dc", Ph="b0a080",
        Pill="ede8dc", NBg="fffdf7",
        NInfo="4878b8", NSucc="387830", NWarn="906010", NErr="b03020",
        SliderTrack="d8d0bc", SliderFill="805828",
    },
    -- Ice / arctic
    Ice = {
        Bg="f0f8ff", Top="e0f0fc", Side="e4f2fc",
        Card="ffffff", CardH="e8f6ff", Stroke="b8daf0",
        Text="081828", Dim="5880a0", SecLbl="1878c8",
        Acc="1878c8", AccD="0c5090",
        TOn="1878c8", TOff="b0d4ee", Knob="ffffff",
        TBOn="1878c8", TBOff="f0f8ff", TxtOn="ffffff", TxtOff="7098b8",
        Div="b8daf0", InBg="e0f0fc", Ph="80aac8",
        Pill="e0f0fc", NBg="ffffff",
        NInfo="1878c8", NSucc="189858", NWarn="c87018", NErr="c83020",
        SliderTrack="b8daf0", SliderFill="1878c8",
    },
...
function Vula:CreateWindow(opts)
    ...
    -- Shadow
    local shadow=ni("Frame",sg,{Size=UDim2.new(0,WW+24,0,WH+24),Position=UDim2.new(.5,3,.5,-38+6),AnchorPoint=Vector2.new(.5,.5),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.92,ZIndex=1}); C(shadow,18)

    -- Main
    local Main=ni("Frame",sg,{Name="Main",Size=UDim2.new(0,WW,0,WH),Position=UDim2.new(.5,3,.5,-38),AnchorPoint=Vector2.new(.5,.5),BackgroundColor3=th.Bg,_bg="Bg",ZIndex=2,ClipsDescendants=true})
    C(Main,14)   -- ← rounder main window
    St(Main,th.Stroke,1,.15)

    -- Corner glow accent
    local ac=ni("Frame",Main,{Size=UDim2.new(0,60,0,60),Position=UDim2.new(0,-18,0,-18),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.9,ZIndex=2}); C(ac,30)

    -- ── Topbar ────────────────────────────────────────────────────────────────
    local TB=ni("Frame",Main,{Size=UDim2.new(1,0,0,TOP_H),BackgroundColor3=th.Top,_bg="Top",ZIndex=5})
    C(TB,14)
    ni("Frame",TB,{Size=UDim2.new(1,0,.5,0),Position=UDim2.new(0,0,.5,0),BackgroundColor3=th.Top,_bg="Top",ZIndex=5})
    ni("Frame",TB,{Size=UDim2.new(1,0,0,1),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.05,ZIndex=8})
    ni("Frame",TB,{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-1),BackgroundColor3=th.Div,_bg="Div",BackgroundTransparency=.3,ZIndex=6})

    -- Badge
    local bdg=ni("Frame",TB,{Size=UDim2.new(0,22,0,22),Position=UDim2.new(0,10,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=6}); C(bdg,11)
    ...
    ni("TextLabel",TB,{Size=UDim2.new(1,-110,0,18),Position=UDim2.new(0,37,0,4),BackgroundTransparency=1,Text=title,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamBold,TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
    local subLbl=ni("TextLabel",TB,{Size=UDim2.new(1,-110,0,11),Position=UDim2.new(0,37,0,23),BackgroundTransparency=1,Text="v4.1 · "..tname,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamMedium,TextSize=8,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
    ...
    -- ── Sidebar ───────────────────────────────────────────────────────────────
    local SB=ni("Frame",Main,{Size=UDim2.new(0,SIDE_W,1,-TOP_H),Position=UDim2.new(0,0,0,TOP_H),BackgroundColor3=th.Side,_bg="Side",ZIndex=4})
    ni("Frame",SB,{Size=UDim2.new(0,1,1,0),Position=UDim2.new(1,-1,0,0),BackgroundColor3=th.Div,_bg="Div",BackgroundTransparency=.25,ZIndex=5})
    ...
    -- ── Loading ───────────────────────────────────────────────────────────────
    local LD=ni("Frame",Main,{Size=UDim2.new(1,0,1,0),BackgroundColor3=th.Bg,_bg="Bg",ZIndex=25}); C(LD,14)
    local ldb=ni("Frame",LD,{Size=UDim2.new(0,44,0,44),Position=UDim2.new(.5,0,.32,0),AnchorPoint=Vector2.new(.5,.5),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=26}); C(ldb,22)
    ...
    ni("TextLabel",LD,{Size=UDim2.new(.9,0,0,18),Position=UDim2.new(.05,0,.44,0),BackgroundTransparency=1,Text=loadT,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamBold,TextSize=12,ZIndex=26})
    ni("TextLabel",LD,{Size=UDim2.new(.9,0,0,14),Position=UDim2.new(.05,0,.56,0),BackgroundTransparency=1,Text=loadS,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamMedium,TextSize=9,ZIndex=26})
    local lbg=ni("Frame",LD,{Size=UDim2.new(.65,0,0,2),Position=UDim2.new(.175,0,.67,0),BackgroundColor3=th.Div,_bg="Div",ZIndex=26}); C(lbg,1)
    local lfl=ni("Frame",lbg,{Size=UDim2.new(0,0,1,0),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=27}); C(lfl,1)
    ...
    local tabBar = ni("Frame", SideTab, {
        ...
        AnchorPoint=Vector2.new(0,.5), BackgroundColor3=th.Acc,_bg="Acc",
    ...
    local pillDot = ni("Frame", SideTab, {
        ...
        AnchorPoint=Vector2.new(.5,0), BackgroundColor3=th.Acc,_bg="Acc",
    ...
    local arrowLbl = ni("TextLabel", SideTab, {
        ...
        TextColor3=th.Acc,_tc="Acc", Font=Enum.Font.GothamBold,
    ...
    local function selTab(idx)
        ...
        for i,t in ipairs(_tabs) do
            ...
            if i==idx then
                tw(b,  {BackgroundColor3=th.TBOn,_bg="TBOn", BackgroundTransparency=0},   .2)
                if aBg then tw(aBg,{BackgroundTransparency=.88},.2) end
                if lbl then tw(lbl,{TextColor3=th.TxtOn,_tc="TxtOn", TextTransparency=0},.16) end
                if bar then tw(bar,{BackgroundTransparency=0,BackgroundColor3=th.Acc,_bg="Acc"},.16) end
                if ico then tw(ico,{ImageColor3=th.Acc,_ic="Acc"},.16) end
            else
                tw(b,  {BackgroundColor3=th.TBOff,_bg="TBOff", BackgroundTransparency=.72}, .2)
                if aBg then tw(aBg,{BackgroundTransparency=1},.2) end
                if lbl then tw(lbl,{TextColor3=th.TxtOff,_tc="TxtOff", TextTransparency=.25},.16) end
                if bar then tw(bar,{BackgroundTransparency=1},.16) end
                if ico then tw(ico,{ImageColor3=th.TxtOff,_ic="TxtOff"},.16) end
            end
        end
        _act=idx
    end
    ...
    function Win:SetTheme(name)
        ...
        local D = .26
        -- Walk every descendant and apply colors by stored theme-key attribute
        for _, inst in ipairs(sg:GetDescendants()) do
            pcall(function()
                local bgKey = inst:GetAttribute("_thBg")
                local tcKey = inst:GetAttribute("_thTc")
                local scKey = inst:GetAttribute("_thSc")
                local icKey = inst:GetAttribute("_thIc")
                if bgKey and newTh[bgKey] then tw(inst,{BackgroundColor3=newTh[bgKey]},D) end
                if tcKey and newTh[tcKey] then tw(inst,{TextColor3=newTh[tcKey]},D) end
                if scKey and newTh[scKey] and inst:IsA("UIStroke") then tw(inst,{Color=newTh[scKey]},D) end
                if icKey and newTh[icKey] then tw(inst,{ImageColor3=newTh[icKey]},D) end
            end)
        end
        -- Also update SideTab accent bar and arrow which reference th.Acc/th.Pill directly
        pcall(function()
            tw(SideTab,  {BackgroundColor3=newTh.Pill}, D)
            tw(tabBar,   {BackgroundColor3=newTh.Acc},  D)
            tw(arrowLbl, {TextColor3=newTh.Acc},        D)
            tw(pillDot,  {BackgroundColor3=newTh.Acc},  D)
            tw(shadow,   {BackgroundColor3=newTh.Acc},  D)
        end)
    ...
    function Win:CreateTab(name, icon)
        ...
        local bar=ni("Frame",btn,{Name="_b",Size=UDim2.new(0,3,.52,0),Position=UDim2.new(0,0,.24,0),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=first and 0 or 1,ZIndex=7}); C(bar,2)
        local aBg=ni("Frame",btn,{Name="_abg",Size=UDim2.new(1,0,1,0),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=first and .88 or 1,ZIndex=6}); C(aBg,10)
        ...
        function tab:CreateSection(s)
            ...
            local dot=ni("Frame",sf,{Size=UDim2.new(0,4,0,4),Position=UDim2.new(0,2,0,9),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=5}); C(dot,2)
            ni("TextLabel",sf,{Size=UDim2.new(1,-14,0,14),Position=UDim2.new(0,10,0,4),BackgroundTransparency=1,Text=s:upper(),TextColor3=th.SecLbl,_tc="SecLbl",Font=Enum.Font.GothamBold,TextSize=8,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=5})
            local dl=ni("Frame",sf,{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-1),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.55,ZIndex=5})
        ...
        function tab:CreateToggle(o)
            ...
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,36),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo()})
            C(row,9); row:SetAttribute("origH",36)  -- ← rounder cards
            local rSt=St(row,th.Stroke,1,.26)
            local la=ni("Frame",row,{Size=UDim2.new(0,3,.46,0),Position=UDim2.new(0,0,.27,0),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=val and .14 or 1,ZIndex=6}); C(la,2)
            local dot=ni("Frame",row,{Size=UDim2.new(0,5,0,5),Position=UDim2.new(0,12,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=val and .1 or 1,ZIndex=7}); C(dot,3)
            ...
            local knob=ni("Frame",pill,{Size=UDim2.new(0,KS,0,KS),Position=UDim2.new(0,val and K1 or K0,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.Knob,_bg="Knob",ZIndex=7}); C(knob,KS//2)
            ...
            hit.MouseEnter:Connect(function() tw(row,{BackgroundColor3=th.CardH,_bg="CardH"},.13); tw(rSt,{Transparency=.13},.13) end)
            hit.MouseLeave:Connect(function() tw(row,{BackgroundColor3=th.Card,_bg="Card"},.13); tw(rSt,{Transparency=val and .4 or .26},.13) end)
        ...
        function tab:CreateButton(o)
            ...
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,ROW_H),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo(),ClipsDescendants=true})
            C(row,9); row:SetAttribute("origH",ROW_H)  -- ← rounder
            local rSt=St(row,th.Stroke,1,.24)
            local fill=ni("Frame",row,{Size=UDim2.new(0,0,1,0),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.88,ZIndex=5}); C(fill,9)
            ni("TextLabel",row,{Size=UDim2.new(1,-28,0,18),Position=UDim2.new(0,10,0,sub~="" and 5 or 0),BackgroundTransparency=1,Text=bN,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamSemibold,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
            if sub~="" then ni("TextLabel",row,{Size=UDim2.new(1,-28,0,14),Position=UDim2.new(0,10,0,22),BackgroundTransparency=1,Text=sub,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamMedium,TextSize=8,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6}) end
            -- Better arrow: › instead of >
            local arr=ni("TextLabel",row,{Size=UDim2.new(0,16,1,0),Position=UDim2.new(1,-18,0,0),BackgroundTransparency=1,Text="›",TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamBold,TextSize=16,ZIndex=6})
            local hit=ni("TextButton",row,{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="",ZIndex=9,AutoButtonColor=false})
            hit.MouseEnter:Connect(function() tw(row,{BackgroundColor3=th.CardH,_bg="CardH"},.13); tw(fill,{Size=UDim2.new(1,0,1,0)},.24); tw(rSt,{Color=th.Acc,_sc="Acc",Transparency=.5},.16); tw(arr,{TextColor3=th.Acc,_tc="Acc"},.13) end)
            hit.MouseLeave:Connect(function() tw(row,{BackgroundColor3=th.Card,_bg="Card"},.13); tw(fill,{Size=UDim2.new(0,0,1,0)},.16); tw(rSt,{Color=th.Stroke,_sc="Stroke",Transparency=.24},.16); tw(arr,{TextColor3=th.Dim,_tc="Dim"},.13) end)
            hit.MouseButton1Down:Connect(function(mx,my)
                local abs=row.AbsolutePosition
                local rip=ni("Frame",row,{Size=UDim2.new(0,0,0,0),Position=UDim2.new(0,mx-abs.X,0,my-abs.Y),AnchorPoint=Vector2.new(.5,.5),BackgroundColor3=th.Acc,_bg="Acc",BackgroundTransparency=.72,ZIndex=10}); C(rip,999)
        ...
        function tab:CreateLabel(text)
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,26),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo()})
            C(row,9); St(row,th.Stroke,1,.38); row:SetAttribute("origH",26)
            local lbl=ni("TextLabel",row,{Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,8,0,0),BackgroundTransparency=1,Text=text,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamMedium,TextSize=9,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
        ...
        function tab:CreateKeybind(o)
            ...
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,34),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo()})
            C(row,9); St(row,th.Stroke,1,.24); row:SetAttribute("origH",34)
            ni("TextLabel",row,{Size=UDim2.new(1,-80,1,0),Position=UDim2.new(0,10,0,0),BackgroundTransparency=1,Text=kN,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamSemibold,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
            local chip=ni("TextButton",row,{Size=UDim2.new(0,64,0,20),Position=UDim2.new(1,-70,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.InBg,_bg="InBg",Text=cur,TextColor3=th.Acc,_tc="Acc",Font=Enum.Font.GothamBold,TextSize=8,ZIndex=7,AutoButtonColor=false}); C(chip,6)
            local cSt=St(chip,th.Acc,1,.4)
            chip.MouseButton1Click:Connect(function()
                if lst then return end; lst=true; chip.Text="…"
                tw(chip,{BackgroundColor3=th.Acc,_bg="Acc"},.14); tw(cSt,{Transparency=0},.14)
            end)
            UIS.InputBegan:Connect(function(i,gpe)
                if not lst then return end
                if i.UserInputType~=Enum.UserInputType.Keyboard then return end
                lst=false; cur=i.KeyCode.Name; chip.Text=cur
                tw(chip,{BackgroundColor3=th.InBg,_bg="InBg"},.14); tw(cSt,{Transparency=.4},.14)
        ...
        function tab:CreateSlider(o)
            ...
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,44),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo()})
            C(row,9); St(row,th.Stroke,1,.24); row:SetAttribute("origH",44)
            ni("TextLabel",row,{Size=UDim2.new(.62,0,0,16),Position=UDim2.new(0,10,0,6),BackgroundTransparency=1,Text=sN,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamSemibold,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
            local valBg=ni("Frame",row,{Size=UDim2.new(0,40,0,16),Position=UDim2.new(1,-46,0,6),BackgroundColor3=th.InBg,_bg="InBg",ZIndex=6}); C(valBg,6)
            St(valBg,th.Acc,1,.52)
            local valLbl=ni("TextLabel",valBg,{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text=tostring(val)..sfx,TextColor3=th.Acc,_tc="Acc",Font=Enum.Font.GothamBold,TextSize=9,ZIndex=7})
            local TS2=10
            local trackBg=ni("Frame",row,{Size=UDim2.new(1,-TS2*2,0,4),Position=UDim2.new(0,TS2,1,-10),AnchorPoint=Vector2.new(0,1),BackgroundColor3=th.SliderTrack,_bg="SliderTrack" or th.Div,ZIndex=6}); C(trackBg,2)
            local trackFill=ni("Frame",trackBg,{Size=UDim2.new(0,0,1,0),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=7}); C(trackFill,2)
            local KS2=14
            local thumb=ni("Frame",trackBg,{Size=UDim2.new(0,KS2,0,KS2),AnchorPoint=Vector2.new(.5,.5),Position=UDim2.new(0,0,.5,0),BackgroundColor3=th.Acc,_bg="Acc",ZIndex=8}); C(thumb,KS2//2)
            ...
            row.MouseEnter:Connect(function() tw(row,{BackgroundColor3=th.CardH,_bg="CardH"},.13) end)
            row.MouseLeave:Connect(function() tw(row,{BackgroundColor3=th.Card,_bg="Card"},.13) end)
        ...
        function tab:CreateInput(o)
            ...
            local row=ni("Frame",page,{Size=UDim2.new(1,0,0,52),BackgroundColor3=th.Card,_bg="Card",ZIndex=5,LayoutOrder=eo()})
            C(row,9); St(row,th.Stroke,1,.24); row:SetAttribute("origH",52)
            ni("TextLabel",row,{Size=UDim2.new(.75,0,0,14),Position=UDim2.new(0,10,0,5),BackgroundTransparency=1,Text=iN,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamBold,TextSize=9,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
            local inputBg=ni("Frame",row,{Size=UDim2.new(1,-16,0,24),Position=UDim2.new(0,8,0,22),BackgroundColor3=th.InBg,_bg="InBg",ZIndex=6}); C(inputBg,7)
            local iSt=St(inputBg,th.Stroke,1,.26)
            local tb=ni("TextBox",inputBg,{Size=UDim2.new(1,-12,1,0),Position=UDim2.new(0,7,0,0),BackgroundTransparency=1,Text="",PlaceholderText=iPh,TextColor3=th.Text,_tc="Text",PlaceholderColor3=th.Ph,Font=Enum.Font.GothamMedium,TextSize=9,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=7,ClearTextOnFocus=false})
            tb.Focused:Connect(function() tw(iSt,{Color=th.Acc,_sc="Acc",Transparency=.16},.14); tw(inputBg,{BackgroundColor3=th.Card,_bg="Card"},.14) end)
            tb.FocusLost:Connect(function(enter)
                if numO then local n=tonumber(tb.Text); tb.Text=n and tostring(n) or "" end
                tw(iSt,{Color=th.Stroke,_sc="Stroke",Transparency=.26},.14); tw(inputBg,{BackgroundColor3=th.InBg,_bg="InBg"},.14)
        ...
        function tab:CreateDropdown(o)
            ...
            local hdr=ni("Frame",wrap,{Size=UDim2.new(1,0,0,HDRH),BackgroundColor3=th.Card,_bg="Card",ZIndex=5}); C(hdr,9)
            local hdrSt=St(hdr,th.Stroke,1,.24)
            ni("TextLabel",hdr,{Size=UDim2.new(1,-42,1,0),Position=UDim2.new(0,10,0,0),BackgroundTransparency=1,Text=dN,TextColor3=th.Text,_tc="Text",Font=Enum.Font.GothamSemibold,TextSize=10,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6})
            local cntBg=ni("Frame",hdr,{Size=UDim2.new(0,26,0,16),Position=UDim2.new(1,-34,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.InBg,_bg="InBg",ZIndex=7}); C(cntBg,6)
            local cntLbl=ni("TextLabel",cntBg,{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="0",TextColor3=th.Acc,_tc="Acc",Font=Enum.Font.GothamBold,TextSize=8,ZIndex=8})
            -- ▾ / ▴ instead of "v"
            local chev=ni("TextLabel",hdr,{Size=UDim2.new(0,14,1,0),Position=UDim2.new(1,-16,0,0),BackgroundTransparency=1,Text="▾",TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamBold,TextSize=12,ZIndex=7})
            local iFrame=ni("Frame",wrap,{Size=UDim2.new(1,0,0,#opts2*ITMH+6),Position=UDim2.new(0,0,0,HDRH+1),BackgroundColor3=th.InBg,_bg="InBg",ZIndex=5}); C(iFrame,9)
            ...
            for _,optName in ipairs(opts2) do
                local name=optName
                local ib=ni("TextButton",iFrame,{Size=UDim2.new(1,0,0,ITMH),BackgroundColor3=th.Card,_bg="Card",BackgroundTransparency=.72,Text="",ZIndex=6,AutoButtonColor=false}); C(ib,6)
                local chk=ni("Frame",ib,{Size=UDim2.new(0,10,0,10),Position=UDim2.new(0,6,.5,0),AnchorPoint=Vector2.new(0,.5),BackgroundColor3=th.TOff,_bg="TOff",ZIndex=7}); C(chk,4)
                St(chk,th.Acc,1,.3)
                -- ✓ checkmark
                ...
                ni("TextLabel",ib,{Size=UDim2.new(1,-24,1,0),Position=UDim2.new(0,22,0,0),BackgroundTransparency=1,Text=name,TextColor3=th.Dim,_tc="Dim",Font=Enum.Font.GothamMedium,TextSize=9,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=7})
            ...
            hHit.MouseEnter:Connect(function() tw(hdr,{BackgroundColor3=th.CardH,_bg="CardH"},.13) end)
            hHit.MouseLeave:Connect(function() tw(hdr,{BackgroundColor3=th.Card,_bg="Card"},.13) end)
